            ...
            <div class="grid">
                <div class="grid__row" id="release__header">
                    <div class="release__heading">
                        <h1>The New Songs Of 2022</h1>
                    </div>
                    <div class="release__imgheading">
                    </div>
                    <div class="release__caption">
                        <span>Contrary to popular belief/Top 40 radio/your ever-growing collection of used Rolling Stones vinyl, there is still plenty of great new music being released on the regular. The first half of 2022 has shown as much: everything from grungy garage rock to funky self-worth anthems to sun-bright Afrobeats-indebted rhythms to emotional electropop. This periodically updated monthly list, from Vulture staff writer Justin Curto and music editor Alex Suskind, runs down the best we've heard so far this year — a mix of fun, challenging, cathartic new singles we'll be playing on repeat throughout 2022, listed in chronological order by release date.</span>
                    </div>
                </div>

                <?php
                    $sql = "SELECT * FROM new_release where topic = 'newrelease'";
                    $viet = $conn -> query($sql);
                    ...
                    $start = ($page-1)*8;
                    $sql = sprintf("SELECT * FROM new_release where topic = 'newrelease' limit $start,8");
                    $viet = $conn->query($sql);
...
<script>
    function page(x){
        window.location.href = "Newrelease.php?page="+x;
    }
